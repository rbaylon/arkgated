...
// startTestDaemon writes a minimal valid router config.json into storeAt's
// rundir and starts run() against a single real TCP listener, returning its
// address. Reuses storeAt/fakeSrvcman from main_test.go, whose fake srvcman
// answers query/create fast, so run()'s startup Enroll/PfCreate calls don't
...
func startTestDaemon(t *testing.T) (addr string) {
	...
	if err := os.WriteFile(filepath.Join(rundir, "config.json"), []byte(cfgJSON), 0640); err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		run(store, io.Discard, []net.Listener{ln}, ctx)
		close(done)
	}()
...
// source for the dashboard's count.
...
// ClearHealthChecks must actually terminate a stuck HealthPing and
...
// Regression: malformed input must still get NOK and close promptly.
...
// and something genuine for ClearHealthChecks to kill.
